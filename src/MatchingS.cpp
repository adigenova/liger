...
GraphS* MatchingS::matching_cover(GraphS* g) {
    ...
    MaxWeightedMatching<ListGraph, ListGraph::EdgeMap<int> > mwpm(bp, wt);
    mwpm.run();
    ...
    for (ListGraph::EdgeIt e(bp); e != INVALID; ++e) {
        ...
        if(mwpm.matching(e)) {
            ...
            //cout <<"Selected edge:"<<eg->getEdge()<<" "<<eg->getEdge_id()<<" "<<eg->getBw()<<" "<<eid[e]<<endl;
        }else{
            //cout <<"None selected edge:"<<eg->getEdge()<<" "<<eg->getEdge_id()<<" "<<eg->getBw()<<" "<<eid[e]<<endl;
            bp.erase(e);
        }
    }
    ...
    if(!acyclic(bp)){
        ...
        if(!acyclic(bp)){
            cout << "ERROR in Matching COVER: at this point we removed all the cicles, however we found cicles still"<<endl;
            exit(1);
        }
    }else{
